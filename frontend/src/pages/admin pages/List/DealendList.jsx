...
sessionStorage.removeItem('token');
  ...
  const handleBackToInstallment = async (item) => {
    try {
      ...
      const sellingResponse = await fetch('http://localhost:8000/dealendReversion/addSelling', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const deleteResponse = await fetch(`http://localhost:8000/dealend/deleteDealend/${item._id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
